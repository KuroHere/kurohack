...
import me.kurohere.kurohack.features.setting.EnumConverter;
...
implements Util {
    ...
    public String config = "kuro/config/";
    public boolean loadingConfig;
    public boolean savingConfig;

    public void loadConfig(String name) {
        this.loadingConfig = true;
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("kuro").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("kuro/" + name + "/")) ? "kuro/" + name + "/" : "kuro/config/";
        kuro.friendManager.onLoad();
    ...
    public void saveConfig(String name) {
        ...
        this.config = "kuro/" + name + "/";
        File path = new File(this.config);
        if (!path.exists()) {
            path.mkdir();
        }
        kuro.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                kuro.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }

    public void init() {
        this.features.addAll(kuro.moduleManager.modules);
        this.features.add(kuro.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        kuro.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            kuro.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    kuro.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                }
                catch (Exception e) {
                    e.printStackTrace();
                }
