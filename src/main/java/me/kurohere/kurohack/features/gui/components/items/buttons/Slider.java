package me.kurohere.kurohack.features.gui.components.items.buttons;

import me.kurohere.kurohack.kuro;
import me.kurohere.kurohack.features.gui.kurohackGui;
import me.kurohere.kurohack.features.gui.components.Component;
...
extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (ClickGui.getInstance().rainbowRolling.getValue().booleanValue()) {
            int color = ColorUtil.changeAlpha(HUD.getInstance().colorMap.get(MathUtil.clamp((int)this.y, 0, this.renderer.scaledHeight)), kuro.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue());
            int color1 = ColorUtil.changeAlpha(HUD.getInstance().colorMap.get(MathUtil.clamp((int)this.y + this.height, 0, this.renderer.scaledHeight)), kuro.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue());
            ...
            RenderUtil.drawRect(this.x, this.y, ((Number)this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float)this.width + 7.4f) * this.partialMultiplier(), this.y + (float)this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? kuro.colorManager.getColorWithAlpha(kuro.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : kuro.colorManager.getColorWithAlpha(kuro.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        }
        kuro.textManager.drawStringWithShadow(this.getName() + " " + "\u00a77" + (this.setting.getValue() instanceof Float ? (Number)((Number)this.setting.getValue()) : (Number)((Number)this.setting.getValue()).doubleValue()), this.x + 2.3f, this.y - 1.7f - (float)kurohackGui.getClickGui().getTextOffset(), -1);
    }
    ...
    public boolean isHovering(int mouseX, int mouseY) {
        for (Component component : kurohackGui.getClickGui().getComponents()) {
