...
extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (ClickGui.getInstance().rainbowRolling.getValue().booleanValue()) {
            ...
            RenderUtil.drawRect(this.x, this.y, this.x + (float)this.width + 7.4f, this.y + (float)this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? kuro.colorManager.getColorWithAlpha(kuro.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : kuro.colorManager.getColorWithAlpha(kuro.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue())) : (!this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515));
        }
        if (this.isListening) {
            kuro.textManager.drawStringWithShadow(this.currentString.getString() + kuro.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float)kurohackGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
        	kuro.textManager.drawStringWithShadow((this.setting.shouldRenderName() ? this.setting.getName() + " " + "\u00a77" : "") + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float)kurohackGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
