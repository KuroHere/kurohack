...
extends Module {
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && Offhand.mc.world.playerEntities.stream().noneMatch(e -> e != Offhand.mc.player && !kuro.friendManager.isFriend((EntityPlayer)e) && Offhand.mc.player.getDistance((Entity)e) <= this.cTargetDistance.getValue().floatValue());
    }
