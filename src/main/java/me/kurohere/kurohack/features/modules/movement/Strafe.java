...
extends Module {
    ...
    public void onMove(MoveEvent event) {
        ...
        if (this.mode.getValue() == Mode.NCP) {
            ...
            if (this.limiter2.getValue().booleanValue() && Strafe.mc.player.onGround && kuro.speedManager.getSpeedKpH() < (double)this.speedLimit2.getValue().floatValue()) {
                this.stage = 2;
            }
            ...
            if (this.stage == 1 && EntityUtil.isMoving()) {
                ...
                if (Strafe.mc.world.getCollisionBoxes((Entity)Strafe.mc.player, Strafe.mc.player.getEntityBoundingBox().offset(0.0, Strafe.mc.player.motionY, 0.0)).size() > 0 || Strafe.mc.player.collidedVertically && this.stage > 0) {
                    this.stage = this.bhop2.getValue() != false && kuro.speedManager.getSpeedKpH() >= (double)this.speedLimit.getValue().floatValue() ? 0 : (Strafe.mc.player.moveForward != 0.0f || Strafe.mc.player.moveStrafing != 0.0f ? 1 : 0);
                }
    ...
    private boolean shouldReturn() {
        return kuro.moduleManager.isModuleEnabled(Freecam.class) || kuro.moduleManager.isModuleEnabled(Phase.class) || kuro.moduleManager.isModuleEnabled(ElytraFlight.class) || kuro.moduleManager.isModuleEnabled(Flight.class);
    }
