...
extends Module {
    ...
    public Setting<String> command = this.register(new Setting<String>("Command", "kurohack.cc"));
    ...
    @Override
    public void onLoad() {
        kuro.commandManager.setClientMessage(this.getCommandMessage());
    }
        ...
        if (event.getStage() == 2) {
            ...
            if (event.getSetting() != null && this.equals(event.getSetting().getFeature())) {
                if (event.getSetting().equals(this.holeThread)) {
                    kuro.holeManager.settingChanged();
                }
                kuro.commandManager.setClientMessage(this.getCommandMessage());
            }
        }
