...
extends Module {
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (this.keyboard.getValue().booleanValue() && Keyboard.getEventKeyState() && !(MCF.mc.currentScreen instanceof kurohackGui) && this.key.getValue().getKey() == Keyboard.getEventKey()) {
            this.onClick();
        }
    }
    ...
    private void onClick() {
        ...
        if (result != null && result.typeOfHit == RayTraceResult.Type.ENTITY && (entity = result.entityHit) instanceof EntityPlayer) {
            if (kuro.friendManager.isFriend(entity.getName())) {
                kuro.friendManager.removeFriend(entity.getName());
                ...
                kuro.friendManager.addFriend(entity.getName());
