...
extends Module {
    ...
    public void onUpdateWalkingPlayerEvent(UpdateWalkingPlayerEvent event) {
        if (event.getStage() == 0 && this.rotate.getValue().booleanValue()) {
            if (this.stay.getValue().booleanValue() && target != null) {
                kuro.rotationManager.lookAtEntity(target);
            }
            this.doKillaura();
        }
    }
    ...
    private void doKillaura() {
        ...
        int wait = this.delay.getValue() == false || EntityUtil.holding32k((EntityPlayer)Killaura.mc.player) && this.teekaydelay.getValue() == false ? 0 : (wait = (int)((float)DamageUtil.getCooldownByWeapon((EntityPlayer)Killaura.mc.player) * (this.tps.getValue() != false ? kuro.serverManager.getTpsFactor() : 1.0f)));
        ...
        if (this.teleport.getValue().booleanValue()) {
            kuro.positionManager.setPositionPacket(Killaura.target.posX, EntityUtil.canEntityFeetBeSeen(target) ? Killaura.target.posY : Killaura.target.posY + (double)target.getEyeHeight(), Killaura.target.posZ, true, true, this.lagBack.getValue() == false);
        }
