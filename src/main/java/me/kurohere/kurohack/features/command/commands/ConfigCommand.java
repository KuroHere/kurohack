...
extends Command {
    ...
    public void execute(String[] commands) {
        ...
        if (commands.length >= 3) {
            switch (commands[0]) {
                case "save": {
                    kuro.configManager.saveConfig(commands[1]);
                    ConfigCommand.sendMessage("\u00a7aConfig has been saved.");
                    break;
                }
                case "load": {
                    kuro.moduleManager.onUnload();
                    kuro.configManager.loadConfig(commands[1]);
                    kuro.moduleManager.onLoad();
